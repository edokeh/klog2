 ...
 */
    ...
    slice = [].slice, push = [].push, toString = Object.prototype.toString, ngMinErr = minErr("ng"), _angular = window.angular, /** @name angular */
    angular = window.angular || (window.angular = {}), angularModule, nodeName_, uid = [ "0", "0", "0" ];
    /**
 ...
 */
    ...
    function FormController(element, attrs) {
        var form = this, parentForm = element.parent().controller("form") || nullFormCtrl, invalidCount = 0, // used to easily determine if we are valid
 ...
 */
   ...
   */
        ...
        var parentForm = $element.inheritedData("$formController") || nullFormCtrl, invalidCount = 0, // used to easily determine if we are valid
        $error = this.$error = {};
        ...
        return {
            ...
            link: function($scope, $element, $attr, ctrl, $transclude) {
                ...
                $scope.$watchCollection(rhs, function ngRepeatAction(collection) {
                    var index, length, previousNode = $element[0], // current position of the node
 ...
 */
    ...
    var selectDirective = [ "$compile", "$parse", function($compile, $parse) {
        ...
        return {
            ...
            link: function(scope, element, attr, ctrls) {
                ...
                emptyOption, // we can't just jqLite('<option>') since jqLite is not smart enough
                ...
                function setupAsOptions(scope, selectElement, ctrl) {
                    ...
                    var displayFn = $parse(match[2] || match[1]), valueName = match[4] || match[6], keyName = match[5], groupByFn = $parse(match[3] || ""), valueFn = $parse(match[2] ? match[1] : valueName), valuesFn = $parse(match[7]), track = match[8], trackFn = track ? $parse(match[8]) : null, // This is an array of array of existing option groups in DOM.
                    ...
                    function render() {
                        ...
                        var optionGroups = {
                            "": []
                        }, optionGroupNames = [ "" ], optionGroupName, optionGroup, option, existingParent, existingOptions, existingOption, modelValue = ctrl.$modelValue, values = valuesFn(scope) || [], keys = keyName ? sortedKeys(values) : values, key, groupLength, length, groupIndex, index, locals = {}, selected, selectedSet = false, // nothing is selected yet
    ...
    define("angular/angularjs/1.2.7/angular-debug", [], function() {
        return angular;
    });
})(window, document);
 ...
 */
        ...
        var START_TAG_REGEXP = /^<\s*([\w:-]+)((?:\s+[\w:-]+(?:\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|[^>\s]+))?)*)\s*(\/?)\s*>/, END_TAG_REGEXP = /^<\s*\/\s*([\w:-]+)[^>]*>/, ATTR_REGEXP = /([\w:-]+)(?:\s*=\s*(?:(?:"((?:[^"])*)")|(?:'((?:[^'])*)')|([^>\s]+)))?/g, BEGIN_TAG_REGEXP = /^</, BEGING_END_TAGE_REGEXP = /^<\s*\//, COMMENT_REGEXP = /<!--(.*?)-->/g, DOCTYPE_REGEXP = /<!DOCTYPE([^>]*?)>/i, CDATA_REGEXP = /<!\[CDATA\[(.*?)]]>/g, // Match everything outside of normal chars and " (quote character)
 ...
 */
        function htmlParser(html, handler) {
            ...
            function parseEndTag(tag, tagName) {
                ...
                for (pos = stack.length - 1; pos >= 0; pos--) if (stack[pos] == tagName) break;
                if (pos >= 0) {
